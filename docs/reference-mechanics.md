...
The user subsequently requested a longer, readable combat payoff: melee immediately ends control, delayed weapons allow retreat until impact (up to five seconds), and mine placement permits five seconds. Weapon and collision damage accumulates while frogs fly, then is revealed and subtracted one frog at a time after settling. Death explosions can extend the chain. The camera follows the action with brief impact slow motion, restrained shake, and individual outcome close-ups. Active traversal has no terrain damage, while high stomps and weapon-driven collisions can hurt. These supersede the initial ten-second retreat interpretation above and are requested behavior for Rebate Attack Force, not additional observations of the reference.
